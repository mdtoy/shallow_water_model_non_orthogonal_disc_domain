...
integer, dimension (3) :: start_vector, count_vector_u, count_vector_v,  &
                          count_vector_h, count_vector_q
integer :: count
...
allocate (x_vector_2(im+1))
allocate (y_vector_1(jm))
allocate (y_vector_2(jm+1))


read (20) (time_vector(nt),     nt = 1,ntm)
read (20) (x_vector_1(i),       i = 1,im  )
read (20) (x_vector_2(i),       i = 1,im+1)
read (20) (y_vector_1(j),       j = 1,jm  )
read (20) (y_vector_2(j),       j = 1,jm+1)



! Create and open the netCDF file
err = NF_CREATE("sh_water_model.1-layer.non_orth.disc_domain.nc", NF_CLOBBER, ncid)
...
err = NF_DEF_DIM(ncid, 'x1_u1', im+1, x2id)
err = NF_DEF_DIM(ncid, 'x2_h', jm, y1id)
err = NF_DEF_DIM(ncid, 'x2_u2', jm+1, y2id)
...
deallocate(fld1_2d)
allocate (fld1_2d((im+1)*jm))
count = 0
do j = 1,jm
   read (20) ( fld1_2d(count+i),        i = 1,im+1 )
   count = count + im+1
end do
err = NF_REDEF(ncid)
...
allocate (fld1_2d((im+1)*jm))
allocate (fld2_2d((im+1)*jm))
allocate (fld3_2d((im+1)*jm))
allocate (fld4_2d(im*(jm+1)))
allocate (fld5_2d(im*(jm+1)))
allocate (fld6_2d(im*(jm+1)))
allocate (fld7_2d((im+1)*(jm+1)))
allocate (fld8_2d((im+1)*(jm+1)))
allocate (fld9_2d(im*jm))
allocate (fld10_2d(im*jm))


! Define count_vectors to be used in the NF_PUT_VARA_DOUBLE function
count_vector_u(1) = im+1
count_vector_u(2) = jm
count_vector_u(3) = 1
count_vector_v(1) = im
count_vector_v(2) = jm+1
count_vector_v(3) = 1
count_vector_q(1) = im+1
count_vector_q(2) = jm+1
count_vector_q(3) = 1
count_vector_h(1) = im
count_vector_h(2) = jm
count_vector_h(3) = 1
...
do nt = 1, ntm
   count = 0
   do j = 1,jm
      read (20) ( fld1_2d(count+i),   i = 1,im+1 )  ! u1_cov
      read (20) ( fld2_2d(count+i),   i = 1,im+1 )  ! u1_cont
      read (20) ( fld3_2d(count+i),   i = 1,im+1 )  ! u
      count = count + im+1
   end do
   count = 0
   do j = 1,jm+1
      read (20) ( fld4_2d(count+i),   i = 1,im )    ! u2_cov
      read (20) ( fld5_2d(count+i),   i = 1,im )    ! u2_cont
      read (20) ( fld6_2d(count+i),   i = 1,im )    ! v
      count = count + im
   end do
   count = 0
   do j = 1,jm+1
      read (20) ( fld7_2d(count+i),   i = 1,im+1 )    ! zeta
      read (20) ( fld8_2d(count+i),   i = 1,im+1 )    ! pv
      count = count + im+1
   end do
   count = 0
   do j = 1,jm
      read (20) ( fld9_2d(count+i),   i = 1,im )      ! h_star
      read (20) ( fld10_2d(count+i),  i = 1,im )      ! h
      count = count + im
   end do
   ...
   err = NF_PUT_VARA_DOUBLE(ncid, varid, start_vector, count_vector_u, &
                            fld1_2d)
   ...
   err = NF_PUT_VARA_DOUBLE(ncid, varid, start_vector, count_vector_v, &
                            fld4_2d)
   ...
   err = NF_PUT_VARA_DOUBLE(ncid, varid, start_vector, count_vector_q, &
                            fld7_2d)
   ...
   err = NF_PUT_VARA_DOUBLE(ncid, varid, start_vector, count_vector_h, &
                            fld9_2d)
