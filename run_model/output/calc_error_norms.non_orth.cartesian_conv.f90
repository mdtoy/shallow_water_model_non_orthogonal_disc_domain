...
err = NF_OPEN("zetas_for_error_norm_calc.non_orth.cartesian_conv.dx_100km.new.nc",NF_NOWRITE,ncid_in)
...
err = NF_INQ_VARID(ncid_in,'zeta_100km',varid)
...
do i =  1, im*jm
   ! Flag masked values
   if ( zeta_conv(i) .gt. 1.E+30_dbl_kind ) then
      print *, "Masked value at i =", i, zeta_conv(i)
      cycle
   end if
